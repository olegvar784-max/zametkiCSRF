...
// Если форма отправлена — проверяем CSRF-токен ПЕРЕД любыми действиями
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // ПРОВЕРКА CSRF-ТОКЕНА — самое важное!
    if (!CsrfGuard::validate($_POST['csrf_token'] ?? '')) {
        http_response_code(403); // 403 Forbidden — доступ запрещён
        die('Ошибка безопасности: неверный CSRF-токен. Попробуйте обновить страницу и отправить форму снова.');
    }
...
<body>
    <h1>📌 Новая заметка</h1>
    <form method="post">
        <!-- CSRF-токен: скрытое поле, защищает от межсайтовой подделки запроса -->
        <?= CsrfGuard::embed() ?>
